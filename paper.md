...
![Co-occurrence character network of "Pride and Prejudice", extracted automatically using Renard. Node size denotes degree, while edge size denotes the number of co-occurrences between two character.](./pp.pdf){#fig:pp_network height=50% }

As an example, Figure \ref{fig:pp_network} shows the co-occurrence character network of the 1813 novel from Jane Austen "Pride and Prejudice" extracted using the Renard pipeline above. Renard uses the NetworkX Python library [@hagberg-2008-networkx] to manipulate graphs, ensuring compatibility with a wide array of tools and formats.
